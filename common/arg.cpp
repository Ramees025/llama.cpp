...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        "backend to use for the multimodal projector (e.g. cpu, cuda, metal, vulkan).\n"
        "if not set, the backend is selected automatically (same GPU backend as the base model, or CPU if --no-mmproj-offload).\n"
        "this allows running the mmproj on a different backend than the base model",
        [](common_params & params, const std::string & value) {
